...
class TicketApp extends React.Component {
    ...
    render() {
        ...
        let views = [React.createElement(Search, {
        ...
        var nav = [];
        if (this.state.view != 0) nav.push(["Search", e => this.goToSearch()]);
        if (this.state.userId != null) {
            nav.push(["Add Event", e => this.goToCreate()]);
            nav.push(["Logout", e => this.logoutUser()]);
        }
        if (this.state.userId == null) nav.push(["Login", e => this.goToUser()]);else nav.push([this.state.username, e => this.goToUser()]);

        if (this.state.windowWidth < 535) nav = nav.map(x => React.createElement(
            "li",
            { key: x[0] },
            " ",
            React.createElement(
                "button",
                { className: "navBtn", onClick: e => this.uncheck(x[1]) },
                " ",
                x[0]
            ),
            " "
        ));else nav = nav.map(x => React.createElement(
            "button",
            { key: x[0], className: "navBtn mainButton", onClick: x[1] },
            " ",
            x[0]
        ));
        ...
        return React.createElement(
            ...
            React.createElement(
                ...
                React.createElement(
                    "div",
                    { className: "col-4-sm logoCol" },
                    React.createElement("img", { className: "header_logo", src: "resource/admitonelogo.png", alt: "oops" })
                ),
                this.state.windowWidth < 535 ? React.createElement(
                    "div",
                    { className: "col-8-sm nav" },
                    React.createElement(
                        "div",
                        ...
                        React.createElement(
                            "div",
                            { className: "menu-content", id: "mcontent" },
                            React.createElement(
                                "ul",
                                null,
                                nav
    ...
    pushBack(f, currentView) {
        ...
        if (this.state.view != currentView) backCopy.push(f);
        return backCopy;
    }
    ...
    goToComparison(items) {
        var resultsArea = 0;
        if (this.state.view == 1) resultsArea = document.getElementsByClassName('row results viewContent')[0];
    ...
    goToCreate(toedit = null) {
        ...
        if (toedit != null) newState.editing = toedit;
            ...
            for (let parameter in param) {
                if (param[parameter] !== '') {
                    if (parameter === "startDateTime") {
                        ...
                        queryCat = queryCat.concat('&' + parameter + '=' + String(startDateTimeCopy));
                        queryCat = queryCat.concat("T00:00:00Z");
                    } else if (parameter === "endDateTime") {
                        let endDateTimeCopy = param[parameter];
                        queryCat = queryCat.concat('&' + parameter + '=' + String(endDateTimeCopy));
                        queryCat = queryCat.concat("T23:59:59Z");
                    } else {
                        queryCat = queryCat.concat('&' + parameter + '=' + param[parameter]);
                    }
                }
            }
    ...
    customSearch(param, currentItems, append, callback) {
        var data = null;
        // Search parameters
        if (param != null) data = param;else data = this.state.parameters;
        ...
        fetch(this.state.host + '/search', {
            ...
            if (response.ok) return response.json();else throw "No Data";
        }).then(json => {
    ...
    addEvents(items, append, callback) {
        if (append) callback(this.state.results.concat(items));else callback(items);
    }
    ...
    deleteCustomEvent(id) {
        ...
        fetch(this.state.host + '/users/' + this.state.userId + '/events/' + id, {
            ...
            if (!response.ok) throw "Error deleting " + id;
    ...
    getEvents(userId, callback) {
        // Get list of events
        var eventList = [];
        fetch(this.state.host + '/users/' + userId).then(response => {
            if (response.ok) {
                return response.json();
            } else throw "No Data";
        }).then(json => {
    ...
    attending(yesno, id, userId, callback) {
        ...
        if (yesno) {
            ...
            fetch(this.state.host + '/events/' + id + '/attending/' + userId, {
                ...
                if (response.ok) return response.json();else throw "Error adding " + id + ", no data";
            }).then(json => {
            ...
            fetch(this.state.host + '/events/' + id + '/attending/' + userId, {
                ...
                if (response.ok) return response.json();else throw "Error deleting event " + id + " from user " + userId + " attending";
            }).then(json => {
                callback(json);
            }).catch(err => console.log(err));
        }
    ...
    whoseAttending(host, id, callback) {
        fetch(host + '/events/' + id + '/attending/').then(response => {
            if (response.ok) {
                return response.json();
            } else throw "No Data";
        }).then(json => {
...
ReactDOM.render(React.createElement(TicketApp, null), document.getElementById('root'));