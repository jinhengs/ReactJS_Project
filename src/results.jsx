...
class ResultsView extends React.Component {
    ...
    toggleCompare(event, checked) {
        if (checked) {
            ...
            this.setState({
                compareList: this.state.compareList.filter(
                    item => item !== event)
            })
        }
    }
        ...
        return (
            <div className="view">
                <div className="row viewTitle">
                    ...
                    <div className="col-4-sm">
                        <button className="selectBtn mainButton" onClick={this.compare}> Compare </button>
                    </div>
                    <div className="col-4-sm">
                        <button className="backBtn mainButton" onClick={(e) => this.props.back()}> Back </button>
                    </div>
                </div>
                <div className="row results viewContent">
                    {(this.props.result.length > 0) ? <ResultGrid
                        events={this.props.result}
                        getResult={this.props.getResult}
                        toggleCompare={this.toggleCompare}
                        showDetail={this.props.showDetail}
                        />
                        : <h2 className="error"> No results, try again </h2> }
                </div>
            </div>
...
class ResultGrid extends React.Component {
    render() {
        ...
        var cards = this.props.events.map((event,index) => (
            ((index < Math.floor(this.props.events.length/xCount) * xCount) < this.props.events.length < xCount)?
                <EventCard
...
class EventCard extends React.Component {
    ...
    render() {
        ...
        return (
            <div className="event__grid" style={styles}>
                <div className="compare__button" onClick={(e) => this.toggleCompare(e)} />
                <div className="bubble">Add to compare list!</div>
                    <div className="event__price"> {this.props.event.parameters.priceRange.split(/[\s,\.]/)[0]}
                    </div>
                    <div className="result__grid" onClick={(e) => this.props.showDetail(this.props.event)} >
                        <div className="result__desc">
                            <div className="event__title">{this.props.event.parameters.title}</div>
                            <div className="event__desc">{ValidLocation(this.props.event)}</div>
                            <div className="event__desc">{this.props.event.parameters.date}</div>
                        </div>
                </div>
            </div>
        )
