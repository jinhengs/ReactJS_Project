class UserView extends React.Component {
     ...
     */
    componentDidMount() { this.initialize(this.props.userId) }
    ...
    deleteEvent(item) {
        ...
        fetch('http://localhost:3000/users/' + this.props.userId + '/events/'+ item._id,
        {
            ...
            if(!response.ok)
                throw "Error deleting " + item._id
            else
                var eventListCopy = this.state.eventList.filter((x) => x._id != item._id)
        ...
        fetch('http://localhost:3000/events/' + item._id + '/attending/' + this.props.userId,
            {
                ...
                if(response.ok)
                    ...
                    throw "Error deleting event " + item._id + " from user " + this.props.userId + " attending"
            }).then(json => {
                var attendingCopy = this.state.attending.filter((x) => x._id != item._id)
...
//              this.goToPage(this.state.attendPage, 1)
    ...
    changePage(num, choice){
      ...
      for(var a = 0; a < epp; a++){
        if(choice == 0){
          if((newIndex + a) > (this.state.eventList.length-1)){
          } else{
            array[a] = this.state.eventList[a + newIndex];
          }
        } else{
          if((newIndex + a) > (this.state.attending.length-1)){
          } else{
    ...
    render () {
        ...
        // Map custom events
        var eventList = this.state.createdPage.slice(0,this.state.eventsPerPage).filter(x => x.userId != "N/A").map(x => 
            (<div key={x._id} className="userEvent event__grid" style={{backgroundImage: `url(${x.optional.imageURL})`}}>
                <div className="row userButtonRow">
                    <button className="xButton mainButton" onClick={() => this.deleteEvent(x)}> x </button>
                    <button className="mainButton" onClick={() => this.editEvent(x)}> edit </button>
                </div>
                <div className="result__grid" onClick={(e) => this.goToDetails(x)} >
                    <div className="result__desc">
                        <div className="event__title">{x.parameters.title}</div>
                        <div className="event__desc">{ValidLocation(x)}</div>
                        <div className="event__desc">{x.parameters.date}</div>
                    </div>
                </div>
            </div>))

        var attendingList = this.state.attendPage.slice(0,this.state.eventsPerPage).filter(x => x.userId != "N/A").map(x => 
            (<div key={x.id_} className="userEvent event__grid" style={{backgroundImage: `url(${x.optional.imageURL})`}}>
                <div className="row userButtonRow">
                    <button className="xButton mainButton" onClick={() => this.removeAttending(x)}> x </button>
                </div>
                <div className="result__grid" onClick={(e) => this.goToDetails(x)} >
                    <div className="result__desc">
                        <div className="event__title">{x.parameters.title}</div>
            ...
            <div className="view">
                <div className="row viewTitle">
                    <div className="col-10-sm">
                        <h3>{this.props.userId==null?"User":this.props.username}</h3>
                    </div>
                ...
                <div className="row viewContent">
                    {(this.props.userId != null)?
                        <div className="row">
                            <div className="col-6">
                                <div className="row userTitle mainTitle">
                                    <h3>Events</h3>
                                </div>
                                <div className="row">
                                    <div className="col-6-sm">
                                        <h3 class="pageNum">Page {this.state.createdPageNum}/{eventsPagesTotal}</h3>
                                    </div>
                                    {eventsPagesTotal > 1?
                                    <div className="col-6-sm">
                                        <button className="prevNext mainButton" onClick={() => this.changePage(1, 0)}> next</button>
                                        <button className="prevNext mainButton" onClick={() => this.changePage(-1, 0)}> prev</button>
                                    </div>
                                    :null}
                                </div>
                                <div className="row">
                            ...
                            <div className="col-6">
                                ...
                                <div className="row">
                                    <div className="col-6-sm">
                                        <h3 class="pageNum">Page {this.state.attendPageNum}/{attendingPagesTotal}</h3>
                                    </div>
                                    {attendingPagesTotal > 1?
                                    <div className="col-6-sm">
                                        <button className="prevNext mainButton" onClick={() => this.changePage(1, 1)}> next</button>
                                        <button className="prevNext mainButton" onClick={() => this.changePage(-1, 1)}> prev</button>
                                    </div>
                                    :null}
                                </div>
                                <div className="row">
                    ...
                    <div className="user__forms">
                        <form className="advancedSearch userLogon" onSubmit={(e) => this.handleSubmit(e, 'loginUser')}>
                            <h6> Log in </h6>
                            <div className="row">
                                <input
                                    name="username"
                            ...
                            <div className="row">
                                <h3> Username taken or password incorrect </h3>
                            </div>
                            :null}
                            <div className="row">
                        ...
                        <form className="advancedSearch userLogon" onSubmit={(e) => this.handleSubmit(e, 'createUser')}>
